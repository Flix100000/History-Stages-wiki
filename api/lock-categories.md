...
¹ Individually only at the stations that know which player is standing at them — crafting table, the 2×2 inventory grid, stonecutter, smithing table. → [Global vs Individual Stages](/docs/next/general/global-vs-individual-stages) has the full list of what a per-player recipe gate does and does not reach.

² Zones are in **beta** — the category is younger than the rest and still growing. Its one scope exception is the "no mob spawns" rule, which is global-only for the same reason `spawnlock` is. → [Zones](/docs/next/modpack-developers/zones).
