...
The recipe reload sits only on the two global methods because recipes are a global-only lock category — a reload per individual unlock would be work for nothing. → [Global vs Individual Stages](/docs/next/general/global-vs-individual-stages) for why that category cannot go per-player.
