...
Requirements sit inside dependency groups on a stage — every entry in a group has to be met before the group counts as satisfied. → [Stage Configuration](../modpack-developers/stage-configuration.md#stage-dependencies) covers the packmaker-facing side of that. Note that a requirement ID is a registry key, not a JSON field name: the built-ins keep their entries in typed fields whose names do not always match (`entity_kill` writes into `entity_kills`, `item_tag` into `item_tags`, `individual_stage` into `individual_stages`). Addon requirements have no such split — their ID is also the key they store under.
