...
*   **Stage Settings:** A dedicated settings screen for managing core stage metadata such as Stage ID, Display Name, and Research Time, keeping these properties separate from the lock entries. The same screen also exposes the [Stage Tier Gating](/docs/modpack-developers/in-game-tools/research-system#stage-tier-gating) controls (`min_pedestal_tier` and `pedestal_tier_mode`), and the [Lose on Death](/docs/modpack-developers/locking-zones/stage-behavior#lose-on-death) toggle for individual stages, so these can be configured without editing JSON.
*   **Folders & Organize Mode:** Stages can be sorted into nested [folders](/docs/modpack-developers/locking-zones/stage-behavior#folders) for large modpacks. A dedicated organize mode on the Stage Overview screen lets you tick multiple stages and folders and drag-and-drop them onto a target folder in one move; moving a folder brings its contents along automatically.
...
→ [Lock Types](/docs/modpack-developers/locking-zones/entity-and-trade-locks#entity-control) has what each of those writes into the stage file, and the four limits worth knowing about extra biomes.
