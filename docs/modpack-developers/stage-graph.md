...
The Stage Graph is **off by default**. A modpack creator enables it by setting `enabled = true` under `[general]` in `graph.toml`, either by hand or through the **Graph** tab of the in-game [Config Editor](./in-game-editor.md#config-editor). Once enabled, a "Stage Graph" button appears on the vanilla pause menu for every player, next to the (still OP-only) editor button.
...
`graph.toml` can be edited entirely in-game via the **Graph** tab of the [Config Editor](./in-game-editor.md#config-editor), alongside the existing Client and Common tabs. Changes save immediately and sync to connected clients, same as the other config tabs.
