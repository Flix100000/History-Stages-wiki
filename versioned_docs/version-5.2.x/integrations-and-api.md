...
| **FTB Quests** | History Stages adds native "History Stage" task and reward types directly into the FTB Quests editor. This allows modpack creators to build progression directly into their questlines. Tasks are event-driven and auto-complete when the corresponding stage is unlocked. Rewards can be configured to either unlock or relock stages. |
...
| **Curios API** | When Curios is present, History Stages extends its item-locking checks to Curio slots, preventing players from equipping locked items into trinket, ring, belt, or other Curios-managed slots. |

## Developer API (For Mod Developers)

For mod developers seeking to extend or interact with History Stages programmatically, the mod fires custom Forge events on the Forge event bus. This allows other mods or scripting frameworks (such as KubeJS or CraftTweaker) to react to stage changes and implement custom logic.
