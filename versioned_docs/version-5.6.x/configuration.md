...
History Stages provides extensive configuration options to fine-tune its behavior, accessible via TOML files located in the `/config` directory. These settings can also be managed through the in-game config editor.

> **Stage Graph:** The player-facing Stage Graph has its own dedicated `graph.toml` file with ~90 keys (canvas appearance, visibility rules, node styling). It is documented separately on the **[Stage Graph](./stage-graph.md)** page and has its own "Graph" tab in the in-game config editor.
...
| `biome_lock.checkInterval` | `10` | Interval in ticks for checking if a player is inside a locked biome. Range 1-200. |
| `biome_lock.effectsEnabled` | `true` | If `true`, applies potion effects to players standing in a locked biome. |
| `biome_lock.effects` | `["minecraft:blindness, 30, 0"]` | List of potion effects applied while inside a locked biome. Format per entry: `"effect_id, seconds, amplifier"`. |
| `biome_lock.clearEffectsOnLeave` | `false` | If `true`, applied effects are cleared as soon as the player leaves the locked biome instead of running their full duration. |
| `biome_lock.messageEnabled` | `true` | If `true`, sends a periodic message while inside a locked biome. |
| `biome_lock.messageFormat` | `"&cYou cannot survive in &e{biome}&c yet!"` | Customizes the format of the biome lock message. Supports `{stage}` and `{biome}` placeholders. |
| `biome_lock.showInChat` | `false` | If `true`, biome lock messages are sent to chat in addition to the actionbar. |
| `biome_lock.damageEnabled` | `true` | If `true`, players take damage while inside a locked biome. |
| `biome_lock.damageAmount` | `1.0` | Amount of damage dealt per damage tick when inside a locked biome. Range 0.1-100.0. |
| `biome_lock.damageInterval` | `20` | Interval in ticks between damage ticks when inside a locked biome. Range 1-600. |
| `biome_lock.blockRightClick` | `true` | Cancel right-click interactions while inside a locked biome. |
| `biome_lock.blockLeftClick` | `true` | Cancel left-click interactions (attacking, mining) while inside a locked biome. |
| `biome_lock.blockProjectiles` | `true` | Cancel projectiles and explosions the moment they would impact something inside a locked biome. |
...
| `scroll_tooltip.lines` | (built-in defaults) | Ordered, per-section layout of the Research Scroll tooltip. See [Research System](./research-system.md#configuring-the-scroll-tooltip). |
| `scroll_tooltip.hideFulfilledDependencies` | `false` | If `true`, dependencies the player already meets are hidden from the scroll tooltip's dependency list. |
| `open_scroll.chapters` | (built-in defaults) | Ordered, per-chapter layout of the [open scroll document](./research-system.md#the-open-scroll-document) (`overview`, `items`, `creatures`, `world`). |
| `open_scroll.lockedDisplay` | `"obscured"` | `"visible"` shows locked entries normally in the open scroll document; `"obscured"` shows them as silhouettes with a redacted name. |
| `open_scroll.overviewBlocks` | (built-in defaults) | Which blocks (`icon`, `title`, `description`, `counts`) appear on the open scroll document's overview page, and in what order. |
| `open_scroll.showSearch` | `true` | If `true`, the open scroll document shows a search bar. |
| `open_scroll.showEntryIds` | `true` | If `true`, entries in the open scroll document show their registry ID alongside the display name. |
| `open_scroll.entrySort` | `"defined"` | `"defined"` keeps entries in the order they're listed in the stage; `"alphabetical"` sorts them by name. |
| `open_scroll.inkHeading` / `inkBody` / `inkFaint` | `#3F2D13` / `#4A3416` / `#7A5A2C` | Ink colors used for headings, body text, and faint/secondary text in the open scroll document. |
