...
The tooltip shown on a Research Scroll is fully configurable via `historystages-common.toml` under `scroll_tooltip.lines` (also editable in-game through a dedicated screen in the [Config Editor](./in-game-editor.md#key-capabilities)). Each entry is a pipe-delimited string: `id|enabled|spacerBefore|style|text`.
...
The related `scroll_tooltip.hideFulfilledDependencies` toggle hides dependency lines the player has already met from the list, so only outstanding requirements are shown.

## The Open Scroll Document

Once a Research Scroll has been researched (or an already-unlocked scroll is opened), it becomes an **open scroll** — a distinct item that, when used, opens a dedicated book-style GUI (the "open scroll document") describing the stage in full: an overview page plus `items`, `creatures`, and `world` (structures/biomes/dimensions) chapters. Locked entries inside the document are shown as silhouettes by default (`open_scroll.lockedDisplay = "obscured"`), or normally if set to `"visible"`.

Chapters, the overview layout, search, entry IDs, sort order, and page ink colors are all configurable under `open_scroll` in `historystages-common.toml` — see [Configuration](./configuration.md). An open scroll can be resealed back into a closed Research Scroll via a `reseal_scroll` recipe.
